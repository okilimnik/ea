...
(def INITIAL-BALANCE (* TRADE-AMOUNT-BTC 10))
(def STOP-PROFIT 20)
(def STOP-LOSS 20)
...
(defn simulate-intraday-trade! [strategy dataset]
  (let [[open-strategy close-strategy] strategy
            ...
            (doseq [k (keys TIMEFRAME->GENE)]
              (let [price-change-percent (get-price-change-percent k @prices-queue price-queue-length)]
                (swap! price-changes assoc k (-> (cond
                                                 ...
                                                 math/round))))
            (let [price (wait-close-possibilty! @price-changes close-strategy (:price @order))]
              (when price
                (swap! evaluation + 0.00001)
                (when @order
                  (swap! evaluation + 0.00002)
                  (let [new-amount-btc (/ (:spent @order) price)
                        comission (* COMISSION new-amount-btc)]
                    (swap! number-of-trades inc)
                    (swap! balance + (- new-amount-btc comission))
                    (reset! order nil)))))
            (let [price (wait-open-possibility! @price-changes open-strategy)]
              (when price
                (swap! evaluation + 0.00001)
                (when-not @order
                  (let [comission (* COMISSION TRADE-AMOUNT-BTC)]
                    (swap! balance - TRADE-AMOUNT-BTC comission))
                  (reset! order {:price price
                                 :spent (* TRADE-AMOUNT-BTC price)})))))
    ...
    (let [final-eval (float (+ @evaluation (* 100 (- @balance INITIAL-BALANCE))))]
      ...
      (println "close-strategy: " close-strategy)
